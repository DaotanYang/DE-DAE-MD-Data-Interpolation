...
Index:
Part                            Location
Data load and preprocessing     Lines 47
DAE1 training process           Lines 153
DAE2 training process           Lines 187
DAE3 training process           Lines 222
Result                          Lines 259

If you want to run this function in your local environment, you only need to change one variable:
path_dataset, on line 51, which you need to change to the path where your data is stored.
...
from models import DAE, DAE2, DAE3, custom_loss, reshape_dae_output, reshape_dae_output_mask, insert_and_impute, insert_and_impute_ensemble
...
if __name__ == "__main__":
    ...
    df_insulation_roof_imputed_ensemble, mask_ensemble = insert_and_impute_ensemble(x_val, x_true, x_train, target_column, 0.2, dae, dae2, dae3, num_iterations = 1)

    # Test the correct rate of the target column of ensemble when the missing ratio is 10%.
    df_insulation_roof_imputed_ensemble, mask_ensemble = insert_and_impute_ensemble(x_val, x_true, x_train, target_column, 0.1, dae, dae2, dae3, num_iterations = 1)
