...
DAE1 training process           Lines 150
DAE2 training process           Lines 192
DAE3 training process           Lines 226
Result                          Lines 260
...
from models import DAE, DAE2, DAE3, custom_loss, insert_and_impute, insert_and_impute_ensemble, reshape_dae_output
...
if __name__ == "__main__":
    ...
    output_dtype(output_missing_rate)

    print('The information about missing values in insulation floor data: \n')
    output_missing_rate(output_missing_rate)

    # preprocessing dataset
    # label encoding and drop NA
    df_ECS_consumption_no_missing_values = encode_dropna(output_missing_rate)
    # encode post_code
    ...
    output_distribution(df_ECS_consumption_no_missing_values_outliers)
    output_heatmap(df_ECS_consumption_no_missing_values_outliers)
    # end preprocessing
    ...
    for epoch in range(num_epochs // iterations_per_noise):
        ...
        df_insulation_ECS_imputed_dedaemd, mask = insert_and_impute(x_val, x_true, x_train, target_column, 0.2, dae, num_iterations = 1)

        # Test the correct rate of the target column of DAE1 when the missing ratio is 10%.
        df_insulation_ECS_imputed_dedaemd, mask = insert_and_impute(x_val, x_true, x_train, target_column, 0.1, dae, num_iterations = 1)
    ...
    df_insulation_ECS_imputed_ensemble, mask_ensemble = insert_and_impute_ensemble(x_val, x_true, x_train, target_column, 0.2, dae, dae2, dae3, num_iterations = 1)

    # Test the correct rate of the target column of ensemble when the missing ratio is 10%.
    df_insulation_ECS_imputed_ensemble, mask_ensemble = insert_and_impute_ensemble(x_val, x_true, x_train, target_column, 0.1, dae, dae2, dae3, num_iterations = 1)
