...
Lines 45-137 are the data preprocessing process.
Lines 140-195 through y are the model training and prediction processes.

If you want to run this function in your local environment, you only need to change two variables:
The first variable is path_dataset, on line 49, which you need to change to the path where your data is stored.

The second variable is save_path, on line 168, which is used to save an image of the resulting loss change. 
...
from models import DAE, insert_and_impute
...
if __name__ == "__main__":
    ...
    output_dtype(df_insulation_roof)

    print('The information about missing values in insulation floor data: \n')
    output_missing_rate(df_insulation_roof)

    # preprocessing dataset
    # label encoding and drop NA
    df_insulation_roof_encoded = encode_dropna(df_insulation_roof)
    # encode post_code
    df_insulation_roof_encoded_no_missing_values = process_postal_code(df_insulation_roof_encoded)
    ...
    output_distribution(df_insulation_roof_encoded_no_missing_values_outliers)
    output_heatmap(df_insulation_roof_encoded_no_missing_values_outliers)
    # end preprocessing
    ...
    loss_history = []
    fig_title = 'DAE Loss'
    save_path = '/content/gdrive/MyDrive/Colab Notebooks/DPE_code/result/dae_result_roof.png'
    ...
    df_insulation_roof_imputed_traditional_dae, mask = insert_and_impute(x_val, x_true, x_train, target_column, 0.2, dae, num_iterations = 1)

    # Test the correct rate of the target column of DAE1 when the missing ratio is 10%.
    df_insulation_roof_imputed_traditional_dae, mask = insert_and_impute(x_val, x_true, x_train, target_column, 0.1, dae, num_iterations = 1)
