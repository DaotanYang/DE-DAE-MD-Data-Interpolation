...
Lines 46-134 are the data preprocessing process.
Lines 135-190 through y are the model training and prediction processes.
...
The second variable is save_path, on line 163, which is used to save an image of the resulting loss change. 
...
if __name__ == "__main__":
    ...
    df_insulation_ECS_imputed_traditional_dae, mask = insert_and_impute(x_val, x_true, x_train, target_column, 0.2, dae, num_iterations = 1)

    # Test the correct rate of the target column of DAE1 when the missing ratio is 10%.
    df_insulation_ECS_imputed_traditional_dae, mask = insert_and_impute(x_val, x_true, x_train, target_column, 0.1, dae, num_iterations = 1)
