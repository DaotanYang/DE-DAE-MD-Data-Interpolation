...
Lines 46-132 are the data preprocessing process.
Lines 133-190 through y are the model training and prediction processes.
...
The second variable is save_path, on line 161, which is used to save an image of the resulting loss change. 
...
if __name__ == "__main__":
    ...
    output_dtype(df_insulation_walls)

    print('The information about missing values in insulation floor data: \n')
    output_missing_rate(df_insulation_walls)

    # preprocessing dataset
    # label encoding and drop NA
    df_insulation_walls_encoded = encode_dropna(df_insulation_walls)
    # encode post_code
    df_insulation_walls_encoded_no_missing_values = process_postal_code(df_insulation_walls_encoded)
    ...
    output_distribution(df_insulation_walls_encoded_no_missing_values_outliers)
    output_heatmap(df_insulation_walls_encoded_no_missing_values_outliers)
    # end preprocessing
    ...
    df_insulation_walls_imputed_dedae, mask = insert_and_impute(x_val, x_true, x_train, target_column, 0.2, dae, num_iterations = 1)

    # Test the correct rate of the target column of DAE1 when the missing ratio is 10%.
    df_insulation_walls_imputed_dedae, mask = insert_and_impute(x_val, x_true, x_train, target_column, 0.1, dae, num_iterations = 1)
